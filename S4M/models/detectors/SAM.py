...
class SAM(DetectionTransformer):
    ...
    def __init__(
        ...
        num_mask_refinements: int = 2,
        forward_all_at_once: bool = False,
        **kwargs
    ):
        bbox_head.update(
            transformer_dim=prompt_encoder["label_encoder"]["embed_dims"]
        )  # noqa
    ...
    def encode_prompts(
        self, batch_data_samples: List[PackPointDetInputs], use_mask_prompt: bool
    ) -> Dict:
        ...
        encoded_prompt_dict = dict(
            ...
            prompt_padding_masks=padding_masks.to(torch.float)
    ...
    def pre_transformer(
        ...
        padding_mask = (
            F.interpolate(padding_mask.unsqueeze(1), size=feat.shape[-2:])
            .to(torch.bool)
            .squeeze(1)
        )
        ...
        decoder_inputs_dict.update(
            dict(img_feats=feat, img_pos=img_pos, padding_mask=padding_mask)
        )
        ...
        if self.use_mask_refinement:
            for i in range(self.num_mask_refinements):
                ...
                for r, b in zip(results_list, batch_data_samples):
                    ...
                    if not isinstance(self.prompt_encoder, ExtremeSAMPaddingGenerator):
                        # replace point prompts with box prompts derived from predicted mask
                        replace_boxes = (
                            b.gt_instances.prompt_types == PromptType.POINT.value
                        )
                        pred_box_prompts = scale_boxes(
                            r["bboxes"], b.metainfo["scale_factor"]
                        ).view(r["bboxes"].shape[0], 2, 2)
                        b.gt_instances.boxes = torch.stack(
                            [
                                pred_b if replace_boxes[ind] else gt_b
                                for ind, (pred_b, gt_b) in enumerate(
                                    zip(pred_box_prompts, b.gt_instances.boxes)
                                )
                            ]
                        )
