...
class SAMTransformerDecoder(BaseModule):
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = ModuleList(
            [
                SAMTransformerLayer(
                    skip_first_layer_pe=(i == 0) and self.skip_first_layer_pe,
                    **self.layer_cfg
                )
    ...
    def forward(
        ...
        attn_out = self.final_attn_token_to_image(
            q, key=k, value=keys, padding_mask=padding_mask
        )
...
class SAMTransformerDecoderMultiInstance(BaseModule):
    """
    A transformer decoder that attends to an input image using
    queries whose positional embedding is supplied.
    ...
    def _init_layers(self) -> None:
        ...
        self.layers_inst = ModuleList(
            [
                SAMTransformerLayer_instanceBIatt(skip_first_layer_pe=False)
        ...
        self.instance_embedding = nn.Embedding(self.num_instances, self.embedding_dim)

    def init_weights(self) -> None:
        """Initialize weights for Transformer and other components."""
        super().init_weights()
        nn.init.xavier_uniform_(self.instance_embedding.weight)
        # nn.init.orthogonal_(self.instance_embedding.weight)
    ...
    def forward(
        ...
        perm = torch.randperm(self.instance_embedding.weight.size(0))  # shuffle indices
        instance_embedding = self.instance_embedding.weight[perm]  # permute weights

        for layer in self.layers_inst:
            queries, keys = layer(
                queries=queries,
                keys=keys,
                query_pe=query_pos,
...
class SAMTransformerLayer(BaseModule):
    ...
    def forward(
        ...
        attn_out = self.cross_attn_image_to_token(
            k, key=q, value=queries, key_padding_mask=prompt_padding_mask
        )
        keys = keys + attn_out
...
class SAMTransformerLayer_instanceBIatt(BaseModule):
    def __init__(
        self,
        num_heads: int = 8,
        embedding_dim: int = 256,
        ...
        # self.norm4 = build_norm_layer(self.norm_cfg, embedding_dim)[1]
        # self.cross_attn_image_to_token = SAMAttention(
        #     embedding_dim, num_heads=num_heads, downsample_rate=attention_downsample_rate, dropout=0.1,
        # )
    ...
    def forward(
        ...
        prompt_attn_mask: Tensor = None,
        prompt_padding_mask: Tensor = None,
        instance_embedding: Tensor = None,
    ) -> Tuple[Tensor, Tensor]:
        B_total, N, C = queries.shape  # B_total = bs * num_instances
        bs = B_total // self.num_instances

        instance_embed = instance_embedding[None, :, None].expand(bs, -1, N, -1)

        # NOTE shuffling instance weight done in transformer, to avoid shuffling per layer
        # perm = torch.randperm(instance_embedding.weight.size(0))  # shuffle indices
        # shuffled_weight = instance_embedding.weight[perm]         # permute weights
        # instance_embed = shuffled_weight[None, :, None].expand(bs, -1, N, -1)

        instance_embed = instance_embed.reshape(bs, self.num_instances * N, C)
        # instance_embed = instance_embedding.weight[None].expand(bs, -1, -1)
        # instance_embed = instance_embed.reshape(bs, self.num_instances*N, C)

        queries = queries.view(bs, self.num_instances, N, C).view(
            bs, self.num_instances * N, C
        )
        prompt_padding_mask_inst = prompt_padding_mask.view(
            bs, self.num_instances, N
        ).view(bs, self.num_instances * N)
        query_pe_inst = query_pe.view(bs, self.num_instances, N, C).view(
            bs, self.num_instances * N, C
        )
        ...
        if self.skip_first_layer_pe:
            queries = self.self_attn(
                queries + instance_embed,
                key=queries + instance_embed,
                value=queries,
                attn_mask=prompt_attn_mask,
                key_padding_mask=prompt_padding_mask_inst,
            )

        else:
            q = queries + query_pe_inst + instance_embed
            attn_out = self.self_attn(
                q,
                key=q,
                value=queries,
        ...
        queries = queries.view(B_total, N, C)
        ...
        # queries = queries.view(bs, self.num_instances, N, C).view(bs, self.num_instances*N, C)
        # keys = keys.reshape(bs, -1, C)
        # key_pe = key_pe.reshape(bs, -1, C)

        # Cross attention block, image embedding attending to tokens
        # NOTE maybe here we need the instance embedding?  No
        # q = queries + query_pe_inst + instance_embed
        # q = queries + query_pe_inst
        # k = keys + key_pe
        # attn_out = self.cross_attn_image_to_token(
        #     k,
        #     key=q,
        ...
        # keys = keys.view(B_total, -1, C)
        # queries = queries.view(B_total, N, C)

        # regular one, no interaction with all the instance
        # Cross attention block, image embedding attending to tokens
        # q = queries + query_pe
        # k = keys + key_pe
        # attn_out = self.cross_attn_image_to_token(
...
class SAMAttention(MultiheadAttention):
    ...
    def __init__(
        self, embed_dims, batch_first=True, downsample_rate=1, **kwargs
    ) -> None:
        orig_dim = embed_dims
        embed_dims = embed_dims // downsample_rate
        super().__init__(
            embed_dims, batch_first=batch_first, kdim=embed_dims + 1, **kwargs
        )
        ...
        self.attn.out_proj = NonDynamicallyQuantizableLinear(
            self.embed_dims, orig_dim, bias=True
        )
